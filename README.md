...
PaddleOCR와 AI를 이용해 PDF 문서를 WordPress 글로 발행하는 흐름은 아래 YouTube 영상에서 확인할 수 있습니다.
...
- `나중에 하기`로 앱을 먼저 둘러볼 수 있지만, 기본 CPU OCR 런타임이 준비되기 전에는 OCR 실행이 차단됩니다.
- GPU OCR 환경 설치와 실행 장치 선택은 앱 진입 후 `Settings > OCR 설정 > OCR 환경 설정`에서 할 수 있습니다.
- AI 초안 생성, Google Vision 보정, WordPress 게시 기능은 사용자가 직접 API 키 또는 연결 정보를 설정해야 합니다.

## 빠른 시작

1. Releases에서 설치형 또는 포터블 파일을 다운로드합니다.
2. 앱을 실행합니다.
3. 최초 실행 환경 준비 화면이 표시되면 `기본 OCR 환경 준비`를 실행합니다.
4. 기본 OCR 환경 준비를 건너뛰고 앱을 먼저 살펴보려면 `나중에 하기`를 선택합니다.
5. 앱 진입 후 기본 OCR 환경을 다시 준비하려면 `Settings > OCR 설정 > OCR 환경 설정`에서 CPU 환경을 설치하거나 재진단합니다.
6. NVIDIA GPU가 있다면 같은 화면에서 GPU 호환성을 확인한 뒤 GPU OCR 환경을 설치합니다.
7. OpenAI, Gemini, Google Vision, WordPress 기능을 사용하려면 `Settings > 연동 설정`에서 필요한 API 키와 연결 정보를 저장합니다.
...
1. `Windows의 PC 보호` 화면에서 `추가 정보`를 선택합니다.
2. 앱 이름을 확인합니다. 게시자는 `알 수 없는 게시자`로 표시될 수 있습니다.
3. 공식 Release 페이지에서 받은 파일이 맞다면 `실행`을 선택합니다.
